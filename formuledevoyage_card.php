...
/**
 *    \file       formuledevoyage_card.php
 *    \ingroup    clienjoyholidays
 *    \brief      Page to create/edit/view formuledevoyage
 */
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('clienjoyholidays', 'formuledevoyage', 'read');
	$permissiontoadd = $user->hasRight('clienjoyholidays', 'formuledevoyage', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('clienjoyholidays', 'formuledevoyage', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('clienjoyholidays', 'formuledevoyage', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('clienjoyholidays', 'formuledevoyage', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->clienjoyholidays->multidir_output[isset($object->entity) ? $object->entity : 1].'/formuledevoyage';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/clienjoyholidays/formuledevoyage_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/clienjoyholidays/formuledevoyage_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-clienjoyholidays page-card');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	*/
	...
	if ($action != 'presend') {
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/clienjoyholidays/formuledevoyage_agenda.php', 1).'?id='.$object->id);
