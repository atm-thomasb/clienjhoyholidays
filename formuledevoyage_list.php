...
/**
 *   	\file       formuledevoyage_list.php
 *		\ingroup    clienjoyholidays
 *		\brief      List page for formuledevoyage
 */
...
require_once __DIR__.'/class/formuledevoyage.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('clienjoyholidays', 'formuledevoyage', 'write');
...
if (!isModEnabled("clienjoyholidays")) {
	accessforbidden('Module clienjoyholidays not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/clienjoyholidays/formuledevoyage_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-clienjoyholidays page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/clienjoyholidays/formuledevoyage_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
