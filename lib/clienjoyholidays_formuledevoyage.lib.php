...
function formuledevoyagePrepareHead($object)
{
	...
	if ($showtabofpagecontact) {
		$head[$h][0] = dol_buildpath("/clienjoyholidays/formuledevoyage_contact.php", 1).'?id='.$object->id;
	...
	if ($showtabofpagenote) {
		if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
			...
			$head[$h][0] = dol_buildpath('/clienjoyholidays/formuledevoyage_note.php', 1).'?id='.$object->id;
	...
	if ($showtabofpagedocument) {
		...
		$upload_dir = $conf->clienjoyholidays->dir_output."/formuledevoyage/".dol_sanitizeFileName($object->ref);
		$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
		$nbLinks = Link::count($db, $object->element, $object->id);
		$head[$h][0] = dol_buildpath("/clienjoyholidays/formuledevoyage_document.php", 1).'?id='.$object->id;
	...
	if ($showtabofpageagenda) {
		$head[$h][0] = dol_buildpath("/clienjoyholidays/formuledevoyage_agenda.php", 1).'?id='.$object->id;
	...
	//	'entity:-tabname:Title:@clienjoyholidays:/clienjoyholidays/mypage.php?id=__ID__'
	//); // to remove a tab
	complete_head_from_modules($conf, $langs, $object, $head, $h, 'formuledevoyage@clienjoyholidays');

	complete_head_from_modules($conf, $langs, $object, $head, $h, 'formuledevoyage@clienjoyholidays', 'remove');
