...
$langs->loadLangs(array("clienjoyholidays@clienjoyholidays", "companies", "other", "mails"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->clienjoyholidays->multidir_output[$object->entity ? $object->entity : $conf->entity]."/formuledevoyage/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('clienjoyholidays', 'formuledevoyage', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-clienjoyholidays page-card_document');
...
$modulepart = 'clienjoyholidays';
$param = '&id='.$object->id;
//$relativepathwithnofile='formuledevoyage/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'formuledevoyage/'.dol_sanitizeFileName($object->ref).'/';
