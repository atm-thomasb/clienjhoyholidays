...
class FormuledeVoyage extends CommonObject
{
	 ...
	 */
	public $table_element = 'clienjoyholidays_formuledevoyage';
	...
	public $fields=array(
		...
		"label" => array("type"=>"varchar(255)", "label"=>"Label", "enabled"=>"1", 'position'=>30, 'notnull'=>0, "visible"=>"1", "alwayseditable"=>"1", "searchall"=>"1", "css"=>"minwidth300", "cssview"=>"wordbreak", "help"=>"Help text", "showoncombobox"=>"2", "validate"=>"1", "Required"=>"1",),
		...
		"destination" => array("type"=>"varchar(128)", "label"=>"Destination", "enabled"=>"1", 'position'=>45, 'notnull'=>0, "visible"=>"1",),
		"date_departure" => array("type"=>"datetime", "label"=>"Date Departure", "enabled"=>"1", 'position'=>500, 'notnull'=>1, "visible"=>"-2",),
		"dateroundtrip" => array("type"=>"datetime", "label"=>"Date Round Trip", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
		"transportmode" => array("type"=>"dictionary", "label"=>"Transport Mode", "enabled"=>"1", 'position'=>60, 'notnull'=>0, "visible"=>"1",),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'clienjoyholidays_formuledevoyageline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_formuledevoyage';
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('clienjoyholidays', 'formuledevoyage', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('clienjoyholidays', 'formuledevoyage_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'formuledevoyage/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'formuledevoyage/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'formuledevoyage/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'formuledevoyage/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->clienjoyholidays->dir_output.'/formuledevoyage/'.$oldref;
				$dirdest = $conf->clienjoyholidays->dir_output.'/formuledevoyage/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->clienjoyholidays->dir_output.'/formuledevoyage/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/clienjoyholidays/formuledevoyage_card.php', 1).'?id='.$this->id;
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('CLIENJOYHOLIDAYS_MYOBJECT_ADDON')) {
			$conf->global->CLIENJOYHOLIDAYS_MYOBJECT_ADDON = 'mod_formuledevoyage_standard';
		}
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_formuledevoyage';
