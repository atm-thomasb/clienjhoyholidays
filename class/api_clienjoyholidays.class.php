...
//		}
		// Modification VMR Global Solutions to include extrafields as search parameters in the API GET call, so we will be able to filter on extrafields
		$sql .= " FROM ".MAIN_DB_PREFIX."clienjoyholidays_formuledevoyage AS t LEFT JOIN ".MAIN_DB_PREFIX."clienjoyholidays_formuledevoyage_extrafields AS ef ON (ef.fk_object = t.rowid)";

//		if ((!DolibarrApiAccess::$user->rights->societe->client->voir && !$socids) || $search_sale > 0) {
//			$sql .= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc"; // We need this table joined to the select in order to filter by sale
//		}

//		$sql .= ' WHERE t.entity IN ('.getEntity('formuledevoyage').')';
//		if ((!DolibarrApiAccess::$user->rights->societe->client->voir && !$socids) || $search_sale > 0) {
...
//		}
		...
		if ($sqlfilterlines) {
			$errormessage = '';
			$sql .= " AND EXISTS (SELECT tl.rowid FROM ".MAIN_DB_PREFIX."clienjoyholidays_formuledevoyagedet AS tl WHERE tl.fk_clienjoyholidays_formuledevoyage = t.rowid";
